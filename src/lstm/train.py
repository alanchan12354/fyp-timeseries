...
def main():
    ...
    run_context = create_run_context(
        ...
        notes="Single-model training run for LSTM.",
    )
    ...
    # 4. Train
    metrics = train_model("LSTM", LSTMModel, tr_load, va_load, (X_te_s, y_te), idx_te, experiment_context=run_context, tuning_notes="Single-model default configuration.", hidden=64, layers=2)
    
    # 5. Save Report
    print(metrics)
    with open(os.path.join(REPORTS_DIR, "metrics_lstm.json"), "w") as f:

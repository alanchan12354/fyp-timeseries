...
def main(config=None, config_dict=None, cli_args=None, **overrides):
    ...
    run_context = create_run_context(
        ...
        notes=runtime_config.run_note or "Single-model training run for LSTM.",
    )
    ...
    print(metrics)
    with open(os.path.join(REPORTS_DIR, "metrics_lstm.json"), "w", encoding="utf-8") as f:
...
if __name__ == "__main__":
    parser = build_runtime_parser("LSTM")
